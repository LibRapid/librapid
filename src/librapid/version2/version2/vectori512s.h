...
static inline Vec64c operator>>(Vec64c const a, int32_t b) {
	__m512i aeven = _mm512_slli_epi16(a,
									  8);                            // even numbered elements of a. get sign bit in position
...
public:
	...
	Vec32us(uint16_t i0, uint16_t i1, uint16_t i2, uint16_t i3, uint16_t i4, uint16_t i5, uint16_t i6, uint16_t i7,
			uint16_t i8, uint16_t i9, uint16_t i10, uint16_t i11, uint16_t i12, uint16_t i13, uint16_t i14,
			uint16_t i15,
			uint16_t i16, uint16_t i17, uint16_t i18, uint16_t i19, uint16_t i20, uint16_t i21, uint16_t i22,
			uint16_t i23,
			uint16_t i24, uint16_t i25, uint16_t i26, uint16_t i27, uint16_t i28, uint16_t i29, uint16_t i30,
			uint16_t i31)
	...
	if constexpr ((flags & perm_perm) != 0) {                             // permutation needed
		...
		if constexpr ((flags & perm_largeblock) != 0) {                   // use larger permutation
			...
			if constexpr ((flags & perm_cross_lane) == 0) {               // no lane crossing. Use pshufb
				...
				__m512i yeven = _mm512_permutex2var_epi16(ro, Vec32s().load(evenmask.a),
														  a);  // destination bytes with even position
				__m512i yodd = _mm512_permutex2var_epi16(ro, Vec32s().load(oddmask.a),
...
static inline Vec64c lookup128(Vec64c const index, Vec64c const table1, Vec64c const table2) {
	...
	__m512i ieven2 = _mm512_srli_epi16(index,
									   1);              // even pos bytes of index / 2 (extra bits will be ignored)
	...
	__m512i e2 = _mm512_mask_srli_epi16(e1, me1, e1,
	...
	__m512i o2 = _mm512_mask_slli_epi16(o1, mo1, o1,
										8);        // combined results for odd pos index. get lower 8 bits up if index was even
	__mmask64 maske = 0x5555555555555555;                       // mask for even position
...
static inline Vec64c
lookup256(Vec64c const index, Vec64c const table1, Vec64c const table2, Vec64c const table3, Vec64c const table4) {
#ifdef  __AVX512VBMI__   // AVX512VBMI instruction set not supported yet (April 2019)
...
static inline Vec32s
lookup128(Vec32s const index, Vec32s const table1, Vec32s const table2, Vec32s const table3, Vec32s const table4) {
...
static inline Vec64uc compress_saturated(Vec32us const low, Vec32us const high) {
	...
	__m512i pk = _mm512_packus_epi16(low1,
									 high1);     // this instruction saturates from signed 32 bit to unsigned 16 bit
	__m512i in = constant16ui<0, 0, 2, 0, 4, 0, 6, 0, 1, 0, 3, 0, 5, 0, 7, 0>();
	return _mm512_permutexvar_epi64(in, pk);              // put in right place
}
...
static inline Vec32us compress_saturated(Vec16ui const low, Vec16ui const high) {
	...
	__m512i pk = _mm512_packus_epi32(low1,
									 high1);     // this instruction saturates from signed 32 bit to unsigned 16 bit
	__m512i in = constant16ui<0, 0, 2, 0, 4, 0, 6, 0, 1, 0, 3, 0, 5, 0, 7, 0>();
	return _mm512_permutexvar_epi64(in, pk);              // put in right place
}
...
static inline Vec32s divide_by_i(Vec32s const x) {
	...
	if constexpr (uint16_t(d0) == 0x8000u) {
		return _mm512_maskz_set1_epi16(x == Vec32s((int16_t) 0x8000u),
...
static inline Vec32us divide_by_ui(Vec32us const x) {
	...
	__m512i xm = _mm512_mulhi_epu16(x1,
									multv);                      // high part of 16x16->32 bit unsigned multiplication
