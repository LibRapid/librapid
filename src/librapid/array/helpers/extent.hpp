...
namespace librapid {
	...
	public:
		...
		Extent(const std::vector<T_> &args) : m_dims(args.size()) {
			LR_ASSERT(args.size() <= maxDims,
					  fmt::format("A maximum of {} dimensions are allowed in an Extent object"));
		...
		LR_FORCE_INLINE LR_NODISCARD("") int64_t size() const {
			int64_t res = 1;
			for (int64_t i = 0; i < m_dims; ++i) res *= m_data[i];
			return res;
		}

		LR_FORCE_INLINE LR_NODISCARD("") int64_t dims() const { return m_dims; }
