	...
	void dot(const Array& lhs, const Array& rhs, Array& res)
	{
		...
		if (!res.stride().isTrivial() || !res.stride().isContiguous())
		{
			throw std::invalid_argument(
				"Cannot store result of matrix product in non-trivial result array");
		}
		...
		else
		{
				...
				else
				{
					throw std::invalid_argument(
						"Cannot compute dot product on vectors with " + lhs.extent().str() + " and "
							+
								rhs.extent().str() + " and store the result in Array with "
							+ res.extent().str() +
			...
			else if (lhs.ndim() == 2 && rhs.ndim() == 1)
			{
				...
				{
					throw std::invalid_argument(
						"Cannot compute dot product on matrix and vector with " + lhs.extent().str()
							+ " and " +
							rhs.extent().str() + " and store result in array with "
							+ res.extent().str() +
			...
			else if (lhs.ndim() == 2 && rhs.ndim() == 2)
			{
				...
				{
					throw std::invalid_argument(
						"Cannot compute matrix product on matrices with " + lhs.extent().str()
							+ " and " +
							rhs.extent().str() + " and store the result in an array with "
							+ res.extent().str() +
			...
			auto lhsTemp =
				(lhs.stride().isTrivial() && lhs.stride().isContiguous()) ? lhs : lhs.clone();
			auto rhsTemp =
				(rhs.stride().isTrivial() && rhs.stride().isContiguous()) ? rhs : rhs.clone();
