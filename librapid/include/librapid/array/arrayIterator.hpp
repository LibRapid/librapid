...
namespace librapid::detail {
	...
	template<typename T>
	 ArrayIterator<T>::ArrayIterator(const T &array) :
	...
	template<typename T>
	 ArrayIterator<T>::ArrayIterator(T &&array, IndexType index) :
	...
	template<typename T>
	 auto ArrayIterator<T>::begin() const noexcept
	  -> ArrayIterator<ArrayIterator<T>> {
		return ArrayIterator<ArrayIterator<T>>(*this, 0);
	}

	template<typename T>
	 auto ArrayIterator<T>::end() const noexcept
	  -> ArrayIterator<ArrayIterator<T>> {
